...
class Board:
    ...
    def make_selection(self, chosen_cell):
        ...
        self.show(self.get_paths(chosen_cell))
    ...
    def get_paths(self, cell):
        ...
        for i, j in short_steps:            
            if i in range(cells_number) and j in range(cells_number):
                if self.field[i][j].color == None:                    
                    avlbl_cell = self.field[i][j]
                    self.paths.append([avlbl_cell])
        return self.paths
